...
The generator derives, with **zero** annotation: the path, HTTP method, and `{param}` path params (with the `->where()` regex as `pattern`); `security` from `auth`/`api_key` middleware; a `429` from `rate_limit`; scope prose from `require_scope`; the `fields`/`expand` query params from `#[Fields]`; the **request-body schema** from a typed `RequestData` parameter (plus an auto `422`; with **v2** — framework ≥ 1.58.0 — that DTO's `#[FromRoute]`/`#[FromQuery]` fields are emitted as `path`/`query` parameters and excluded from the body, and `#[ArrayOf]` drives array `items`); and the **success-response schema** from a `ResponseData` / `CollectionResponse` / `PaginatedResponse` return type. In practice **~95% of routes need no annotation at all** — the types are the docs.
