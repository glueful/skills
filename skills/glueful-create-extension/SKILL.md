...
> **First, check the official catalog** (<https://glueful.com/extensions>). Build a custom extension only when no official one fits — the user store / identity & accounts (`glueful/users` — the first-party `UserProviderInterface`), RBAC (`glueful/aegis`), OAuth/SSO (`glueful/entrada`), email (`glueful/email-notification`), push (`glueful/notiva`), SMS/WhatsApp messaging (`glueful/conversa`), search (`glueful/meilisearch`), payments (`glueful/payvia`), runtime concurrency (`glueful/runiva`), image processing (`glueful/media`), edge cache / CDN (`glueful/cdn`), queue supervision / autoscaling (`glueful/queue-ops`), and table archiving (`glueful/archive`) are already covered.
